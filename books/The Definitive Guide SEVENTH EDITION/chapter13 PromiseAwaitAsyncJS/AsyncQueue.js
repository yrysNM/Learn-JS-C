...
class AsyncQueue {
	constructor() {
		...
		// queues the resolve methods for those PRomise are stored here
		this.resolvers = [];

		//Once closed no more values can be enqueued, and more unfulfilled
		// Promise retured
		this.closed = false;
	...
	enqueue(value) {
		...
		if(this.resolvers.length > 0) {
			...
			// if this balue has already been promised resolve that Promise
	...
	dequeue() {
		if(this.values.length >  0) {
			...
			// if no queued and we're closed return a resolved
