...
far. The second
* argument is a number between 0 and 1 specifying how complete
the download
...
async function streamBody(response, reportProgress, processChunk) {
	...
	let bytesRead = 0;						//How many butes received so far 
	...
	while(true) {							//  Loop until we exit below
		...
		if(value) {							// if we hot bute array
			if(processChunk) {				// Process the bytes if

			 	let processed = processChunk(value);  			// a callback we passed
				if(processed) {
					body += processed;
				}
			...
			if(reportProgress) {			// if a progess callback was
