...
//Create a new  server and listen on post 8080
//Connect to http://localhost:8080/ to use it 
let server = new http.Server();
server.listen(8080);


//When the server gets  a new request run this funtion 
server.on("request", (request, response) => {
	//Parse the requested URL
	let pathname = url.parse(request.url).pathname;

	//if the request was for "/" send the client-side chat UI
	if(pathname === "/") {			// Arequest for the chat UI 
	...
	//If the /char request was a GET then a client is connecting
	...
	//Otherwise the /chat request is a POST of a new message
			...
			braodcastNewMessage(request, response);
	}
});
...
function acceptNewClient(request, response) {
	//Remember the response object so we can send future message to it
 	clients.push(response);

	//if the client closes the connection, remove the correcponding
	// response object from the array  ofactive clients
	...
	//set headers  and  send  an intitial chat event to just this one client
...
async function braodcastNewMessage(request,  response) {
	//First read the body of the requst to get the user's message
	...
	//Forma  the message in text/event-srteam format, prefixing each line with data:  
	...
	//Now send this event to all listenong clients
