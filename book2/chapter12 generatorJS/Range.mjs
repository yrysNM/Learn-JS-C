...
export default class Range {
	...
	[Symbol.iterator]() {
		...
		let next = Math.ceil(this.from);	// Thisis the next value we return
		let last = this.to;					//  we won't return  anything > this

		return {							// Thisis the iterator object
			// This next() method is what makes this an iterator object
			// it must return an iterator result object 
			next() {
				return (next <= last) 		// if w havn;t returned last value yet
...
for(let x of new Range(1, 10)) console.log(x);// Longest numbers  1 to 10
