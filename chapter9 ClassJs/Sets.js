...
class NotSet extends  AbstractSet {
	...
	// Our implmentation of the abstract method we  inherited
	has(x) {
		return !this.set.has(x);
	}

	// And wealso ovrride this object method
	toString() {
		return `{x| x ∉ ${this.set.toStirng()} }`;
	}
}
...
class AbstractEnumerableSet extends AbstractSet {
	...
	equals(set) {
		// If th  other set is not also Enumerable it isn;t equal to this one
		if(!(set instanceof AbstractEnumerableSet)) return  false;           

		 // If they  don;t have the same size , they're not equal 
		if(this.size  !== set.size) return false;

		// Loop through th  elements of this set
		for(let element of this) {
			//  If an element isn't in the other set,  they aren;t equal
			if(!set.has(element)) return false;
		}

		// The elements matched , so the setsare equal
		return true;
	}
}
...
class BitSet extends AbstractWritableSet {
	constructor(max) {
		super();
		this.max = max;			// Thmaximum integer  we can store
		this.n =0;				// Howmany integers are in the set
		this.numBytes = Math.floor(max / 8) + 1;			// How many byts we  need
	...
	insert(x) {
		if(this._valid(x)) { 						// if the value is valid
			...
			if(!this._has(byte, bit)) {				// If that bit is nor set yet
				this.data[byte] |= ButSet.bits[bit];// then set it
	...
	remove(x) {
		if(this._valid(x)) {						//  if the valueis valid
	...
	// A getter toreturn the size of the  set
	get size() {
		return this.n;
	}
